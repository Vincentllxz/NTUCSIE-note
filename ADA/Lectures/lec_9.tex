...
\begin{algorithm}[H]
    ...
    Compute residual capacity $c_{R(f)}(u,v)=c(u,v)-f(u,v)$ for all $(u,v)$\;
...
\begin{proof}[correctness]
    ...
    \begin{itemize}
        ...
        \item \red{According to Lemma 5.1.1}: In every round $g$ is a valid flow in $R(f)$, so $f + g$ is a valid $st$-flow in $G$.
